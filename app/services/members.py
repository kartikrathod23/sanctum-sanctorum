...
def create_member(db: Session, data: MemberCreate, now: datetime) -> Member:
    ...
    Email uniqueness is case-insensitive because email is normalized
    by the request schema before reaching this service.
    """
    existing = db.scalar(
...
def list_members(db: Session,limit: int = 20,offset: int = 0) -> MemberPage:
    """List members in id order with pagination."""
    query = select(Member).order_by(Member.id.asc())
    total = db.scalar(select(func.count()).select_from(Member)) or 0
    members = db.scalars(query.limit(limit).offset(offset)).all()
